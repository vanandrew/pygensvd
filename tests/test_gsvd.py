...
def _load_matrices(mattype):
    return tuple(map(np.loadtxt, ("{}/{}.txt".format(mattype, x) for x in ("a", "b", "c", "s", "x", "u", "v"))))
...
def test_square_matrices():
    ...
    outputs = pygsvd.gsvd(matrices[0], matrices[1], full_matrices=True, extras="uv")
...
def test_nonsquare_matrices():
    ...
    for xin, xout in zip(matrices[4:], (x, u, v)):
        assert np.allclose(np.abs(xin[:, :max_size]), np.abs(xout[:, :max_size]))
...
def test_large_matrices():
    ...
    c, s, x, u, v = pygsvd.gsvd(matrices[0], matrices[1], full_matrices=True, extras="uv")
    assert np.allclose(u.dot(np.diag(c)).dot(x.T), matrices[0])
    assert np.allclose(v.dot(np.diag(s)).dot(x.T), matrices[1])
...
def test_return_extras():
    ...
    names = ("a", "b", "c", "s", "x", "u", "v")
    matrices = dict(zip(names, map(np.loadtxt, ("square/{}.txt".format(name) for name in names))))
    extras = "uv"
    ...
    for combo_length in range(len(extras) + 1):
        ...
        for combo in combinations:
            ...
            out = dict(zip(("c", "s", "r") + combo, pygsvd.gsvd(matrices["a"], matrices["b"], extras=ex)))
